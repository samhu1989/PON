...
def accuracy(data,out):
    ...
    #
    loss = {};
    loss['size'] = 0.5*torch.sum( ( ss*srs_gt.data - ss_gt.data )**2 + ( ts*trs_gt.data - ts_gt.data )**2, dim = 1 );
    erot = ( sr1 - sr1_gt.data )**2;
    erot += ( sr2 - sr2_gt.data )**2;
    erot += ( tr1 - tr1_gt.data )**2;
    erot += ( tr2 - tr2_gt.data )**2;
    loss['rot6'] = 0.5*torch.sum( erot, dim=1 );
    loss['bcd'] = 0.5*( box_cd(ss*srs_gt.data,sr1,sr2,ss_gt,sr1_gt,sr2_gt) + box_cd(ts*trs_gt.data,tr1,tr2,ts_gt,tr1_gt,tr2_gt) );
