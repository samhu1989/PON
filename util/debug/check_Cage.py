...
def animate(i,config,net,optim,data):
    ...
    optim.step();
    print('overall:',loss['overall'].data.cpu().numpy());
    print('bce:',loss['bce'].data.cpu().numpy());
    print('recon:',loss['recon'].data.cpu().numpy());
    print('train:',(out['vec']-data[4]).data.cpu().numpy());
    net.train();
    with torch.no_grad():
        out = net(data);
    print('eval:',(out['vec']-data[4]).data.cpu().numpy());
...
def run(**kwargs):
    ...
    anim.save('./log/debug_Cage.gif',writer=writer);
